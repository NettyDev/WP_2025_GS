...
public class Door extends MapSite{
    ...
    public Door(Room room_1, Room room_2) {
        ...
        this.room_1 = room_1;
        this.room_2 = room_2;
        int x1 = room_1.getX(); // współrzędna x pokoju 1
        int y1 = room_1.getY(); // wsp. y pokoju 1
        int x2 = room_2.getX(); // współrzędna x pokoju 2
        int y2 = room_2.getY(); // wsp. y pokoju 2

        if (x2 > x1) { // pokoj leżą jeden obok drugiego - w poziomie
            setX(x2);  // pokój 2 jest po prawej - drzwi mają współrzędne x2,y2
            setY(y2);  // | r1 : r2 |
    ...
    public void draw(Image image) {
        int x = getX();
        int y = getY(); // współrzędne drzwi
        Graphics graphics = image.getGraphics();
        if (direction == Directions.East) { // dwzwi w pionie
